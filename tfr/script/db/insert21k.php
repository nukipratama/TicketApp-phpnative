...
//header('Location:../order');
    ...
    $harga = '150';
    $jumlah = '1';
    book_insert($jenis, $kategori, $harga, $jumlah, $book_email, $bid);
    mailer($name, $jenis, $kategori, $harga, $jumlah, $book_email, $bid);
} else {
    //header('Location:../order');
}

function book_insert($jenis, $kategori, $harga, $jumlah, $book_email, $bid)
{
    require 'db.php';
    try {
        $dbQuery = $dbConn->prepare("INSERT INTO `tuhm_book` (`bid`, `jenis`, `kategori`, `harga_pcs`, `jumlah`, `book_email`) VALUES (?, ?, ?, ?, ?, ?);");
        $dbQuery->bind_param("ssssss", $bid, $jenis, $kategori, $harga, $jumlah, $book_email);
        $dbQuery->execute();
        $dbQuery->close();
